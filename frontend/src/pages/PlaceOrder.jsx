...
const PlaceOrder = () => {
  ...
  const [methon , setMethon] = useState('cod');
  const {navigate , backendUrl , token , cartItems , setCartItems , getCartAmount , delivery_free , products} = useContext(ShopContext)
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
        ...
        <div className='flex gap-3'>
          <input required onChange={onChangeHandler} name='zip' value={formData.zip} type="number" placeholder='Zipcode' className='border border-gray-300 rounded py-1.5 px-3.5 w-full' />
      ...
      <div className='mt-8'>
          ...
          <div className='mt-12'>
        <Title text1={'PAYMENT'} text2={'METHON'} />
        {/* PAyment Methon Selection */}
        <div className='flex gap-3 flex-col lg:flex-row'>
            <div onClick={() => setMethon('stripe')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${methon === 'stripe' ? 'bg-green-400' : ''}`}></p>
              <img className='h-5 mx-4' src={assets.stripe_logo} alt="" />
            </div>
            <div onClick={() => setMethon('cod')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${methon === 'cod' ? 'bg-green-400' : ''}`}></p>
